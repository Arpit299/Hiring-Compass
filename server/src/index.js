...
import {
  analyzeResumeLocal,
  analyzeResumeLocalWithMarketPulse,
...
const MAX_RESUME_TEXT_CHARS = Number(
  process.env.MAX_RESUME_TEXT_CHARS || 50000,
);
...
if (ENFORCE_HTTPS) {
  app.use((req, res, next) => {
    ...
    if (IS_PROD && proto !== "https") {
      return res
        .status(403)
        .json({ success: false, error: "HTTPS is required" });
    }
    next();
  });
}
...
const analyzeLimiter = rateLimit({
  ...
  message: {
    success: false,
    error: "Rate limit exceeded for analysis endpoint.",
  },
});
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!isMimeValid) {
      return cb(
        new Error(
          `Invalid file type: ${file.mimetype || file.originalname}. Only PDF and DOCX files are allowed.`,
        ),
      );
    }
    cb(null, true);
...
async function parseResumeBuffer(file) {
  ...
  try {
    ...
    // unsupported
    throw new Error(
      `Unsupported file type: ${originalname}. Please upload a PDF or DOCX file.`,
    );
  } catch (err) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (req.file) {
        // Parse uploaded file
        console.log(
          `[${requestId}] Processing file: ${req.file.originalname} (${req.file.size} bytes)`,
        );
        try {
          resumeText = (await parseResumeBuffer(req.file)).trim();
        } catch (parseErr) {
          console.error(
            `[${requestId}] File parsing error: ${parseErr.message}`,
          );
        ...
        if (!resumeText || resumeText.length < 50) {
          console.warn(
            `[${requestId}] Parsed resume too short: ${resumeText.length} chars`,
          );
          return res.status(400).json({
            success: false,
            error:
              "Resume content is too short. Please provide at least 50 characters.",
            requestId,
          });
        }
      } else if (
        ...
        if (resumeText.length < 50) {
          console.warn(
            `[${requestId}] Resume text too short: ${resumeText.length} chars`,
          );
          return res.status(400).json({
            success: false,
      ...
      // Perform analysis
      console.log(
        `[${requestId}] Starting analysis for ${jobRole} at ${company}`,
      );
      ...
      const duration = Date.now() - startTime;
      const message =
        error && error.message ? error.message : "Failed to analyze resume";
      console.error(
        `[${requestId}] Analysis error after ${duration}ms: ${message}`,
      );
      ...
      res.status(statusCode).json({
        success: false,
        error:
          IS_PROD && statusCode >= 500 ? "Internal server error" : errorMsg,
        requestId,
        processingTimeMs: duration,
      });
    }
...
app.use((err, req, res, next) => {
  ...
  console.error(
    `[ERROR] ${new Date().toISOString()} - Status: ${statusCode} - ${message}`,
  );
...
server.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    ...
    console.error(
      `  Please close the application using port ${PORT} or use a different port.`,
    );
